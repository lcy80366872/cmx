...
def watch(net,loss):
    layer_name = 'layer1.1.bn2.bn_1'
    for (name, module) in net.named_modules():
        if name == layer_name:
            mou = module
            wandb.watch(mou, log_freq=5, log='all')
    wandb.log({"loss": loss})
def L1_penalty(var):
...
class Solver:
    ...
    def optimize_exchange(self,lam):
        self.net.train()
        self.data2cuda()

        self.optimizer.zero_grad()
        pred,loss = self.net(self.img, self.gps, self.mask[:,0,:,:])


        loss.backward()
        self.optimizer.step()
        batch_iou, intersection, union = self.metrics(self.mask[:,0,:,:].unsqueeze(1), pred)
        return pred, loss.item(), batch_iou, intersection, union
...
def wandb_init(lr,epoch,batch_size,method,lr_method):
    # wandb.login()
    run = wandb.init(
...
class Framework:
    ...
    def fit(self, epochs,lam, no_optim_epochs=5):
        ...
        for epoch in range(1, epochs + 1):
            ...
            train_loss, train_metrics = self.fit_one_epoch(self.train_dl,lam=lam, mode='training')
            val_loss, val_metrics = self.fit_one_epoch(self.validation_dl,lam=lam, mode='val')
            test_loss, test_metrics = self.fit_one_epoch(self.test_dl,lam=lam, mode='testing')
            ...
            print('current best epoch:', epoch_val, ',val g_iou:', val_best_metrics[1], ',test g_iou:',
                  test_best_metrics[1])
            n = 0
            for name, param in self.solver.net.named_parameters():
                if param.requires_grad and name.endswith('weight') and 'bn2' in name:
                    a = param.detach()
                    n = n + 1
                    print('第n层：', n)
                    if n==11 or n==12:
                        print(list(numpy.array(a)))
            # wandb.log({"test_metric": test_metrics[1],
    ...
    def fit_one_epoch(self, dataloader,lam, mode='training'):
        epoch_loss = 0.0
        local_batch_iou = 0.0
        intersection = []
        union = []
        ...
        for i, (img, mask) in progress_bar:
            ...
            if mode == 'training':
                pred_map, iter_loss, batch_iou, samples_intersection, samples_union = self.solver.optimize_exchange(lam=lam)
            else:
